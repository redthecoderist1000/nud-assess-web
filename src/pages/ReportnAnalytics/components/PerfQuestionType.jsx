...
const Question = ({ perf_que_type }) => {
  const questionTypes = perf_que_type;
        ...
        <>
          ...
          <div className="flex flex-col md:flex-row gap-4 w-full">
            {questionTypes.map((q, index) => (
              <div
                key={index}
                className="bg-gray-50 border border-gray-200 rounded-xl p-4 flex-1 min-w-[220px] flex flex-col justify-between"
              >
                <div className="flex justify-between items-center mb-2">
                  <span className="font-semibold text-gray-900 text-[16px]">
                    {q.type}
                  </span>
                  <span className="text-xs bg-white border border-gray-300 rounded px-2 py-1 font-medium">
