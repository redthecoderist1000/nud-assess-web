...
const BTaxonomyAnalysis = ({ data, bloom_tax }) => {
  const bloomData = bloom_tax;

  var totalQuestions = 0;
  if (bloom_tax != null) {
    totalQuestions = bloom_tax.reduce(
  ...
  return (
    ...
    >
      {bloom_tax == null ? (
        <Typography>No Bloom's Taxonomy data available</Typography>
      ) : (
        <>
            ...
            <table className="min-w-full border border-gray-200 rounded-lg">
              ...
              <tbody>
                {bloomData.map((item, index) => (
                  ...
                  >
                    <td className="py-2 px-4">{item.cognitive_level}</td>
